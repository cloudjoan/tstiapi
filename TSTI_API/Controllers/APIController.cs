...
namespace TSTI_API.Controllers
{
    ...
    public class APIController : Controller
    {
        ...
        private SRMain_SREMPCHANGE_OUTPUT SREMPCHANGE_CHANGE(SRMain_SREMPCHANGE_INPUT bean)
        {
            ...
            using (var dbOne = new TSTIONEEntities())
            {
        ...
        public ActionResult ApiSrSatisfyCreate(TB_ONE_SRSatisfy_Survey bean)
        {
            ...
            try
            {
                ...
                //如果意見欄有資訊，就發信給jordan
                if (!string.IsNullOrEmpty(bean.cOpinion))
                {
					...
					string _content = mailBean.MAIL_CONTENT.Replace("[SR_ID]", srBean.cSRID).Replace("[SR_DESC]", srBean.cDesc).Replace("[ENG_NAME]", srBean.cMainEngineerName).Replace("[Q1]", bean.cQuestion1.ToString()).Replace("[Q2]", bean.cQuestion2.ToString()).Replace("[OPINION]", bean.cOpinion);
        ...
        public ActionResult ListDrawer(int PrizeId)
        {
            ...
            var bean = appDB.VIEW_ANNIVERSARY_CHECK_IN.ToList().Where(x => x.DRAW_ID == DrawId && (OverAYearMark == false || AYearAgo >= Convert.ToDateTime(x.REGEST_DATE)) && !x.AT_DESC.Contains("約聘") && !x.AT_DESC.Contains("工讀生"));
